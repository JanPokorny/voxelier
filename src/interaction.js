...
const moved = e => (Math.abs(e.clientX - S.drag.sx) + Math.abs(e.clientY - S.drag.sy)) > 3;
...
function moveDragTo(e) {
  ...
  if (e.shiftKey) {                                  // Shift: adjust height from where it was dragged to
    if (S.drag.shiftAnchorY == null) { S.drag.shiftAnchorY = e.clientY; S.drag.dyBase = S.drag.dy; }
    ...
    const g = groundCell(0); if (g && S.drag.start) { dx = g.x - S.drag.start.x; dz = g.z - S.drag.start.z; }
  }
  ...
  for (const id of S.selection) for (const m of (S.childMeshes[id] || [])) m.position.set(S.drag.dx, S.drag.dy, S.drag.dz);
  overlay.position.set(S.drag.dx, S.drag.dy, S.drag.dz);
}
function commitMove() {
  const x = contextXform(), dL = rotY({ x: Math.round(S.drag.dx), y: 0, z: Math.round(S.drag.dz) }, (4 - x.rot) & 3), dy = Math.round(S.drag.dy);
  for (const id of S.selection) { const c = childById(id); if (c) { c.pos.x += dL.x; c.pos.y += dy; c.pos.z += dL.z; } }
...
function rotDragTo(e) {
  ...
  if (steps !== S.drag.steps) { rotateSelectionBy(steps - S.drag.steps); S.drag.steps = steps; }
}
...
function updateVoxHover() {
  const t = pickVoxel();
  let cell = null;
  if (S.tool === 'add') { cell = t ? t.addCell : voxelTarget(); } else cell = t ? t.cell : null;
...
function startBox(e, base) {
  ...
  S.drag = { ...base, mode: 'box', shiftAnchorY: null, hyBase: 0, box: { x0: s.x, y0: s.y, z0: s.z, x1: s.x, z1: s.z, hy: 0 } };
...
function boxDragTo(e) {
  ...
  if (e.shiftKey) {                                        // vertical extrude (like moving objects with Shift)
    const perPx = (camera.top - camera.bottom) / canvas.getBoundingClientRect().height;
    if (S.drag.shiftAnchorY === null) { S.drag.shiftAnchorY = e.clientY; S.drag.hyBase = b.hy; }
    b.hy = S.drag.hyBase + Math.round((S.drag.shiftAnchorY - e.clientY) * perPx);
...
function renderBox() {
  ...
  const seg = (ax, ay, az, bx, by, bz, len, label) => ({ a: locToW(ax, ay, az), b: locToW(bx, by, bz), mid: locToW((ax + bx) / 2, (ay + by) / 2, (az + bz) / 2), len, filled: true, nolabel: !label });
  const o = [];
...
canvas.addEventListener('pointerdown', e => {
  ...
  if (S.editObject) {
    if (e.button === 0) {
      if (S.tool === 'add' || S.tool === 'erase') { startBox(e, base); }            // box select in the horizontal plane
      else { S.painting = true; S.lastVox = null; applyVoxel(); }                   // paint
    }
  ...
  if (e.button === 0) {
    if (onSel) { S.drag = { ...base, mode: 'move', start: groundCell(0), dx: 0, dy: 0, dz: 0, shiftAnchorY: null, ...moveCollision() }; }
    else S.drag = { ...base, mode: 'pan', clickId: hitId };
  } else if (e.button === 2) {
    if (onSel) S.drag = { ...base, mode: 'rotobj', steps: 0 };
    else { S.drag = { ...base, mode: 'orbit' }; }
  } else if (e.button === 1) { S.drag = { ...base, mode: 'pan', clickId: null, sx: -1e9 }; }  // middle = pan only
});
...
canvas.addEventListener('pointermove', e => {
  ...
  if (measureActive()) {
    if (S.drag) {
      ...
      if (S.drag.mode === 'pan') { S.drag.x = e.clientX; S.drag.y = e.clientY; panCamera(dx, dy); }
      else if (S.drag.mode === 'orbit') { S.drag.x = e.clientX; S.drag.y = e.clientY; orbitView(dx, dy); }
    }
    pointerMeasure(); return;
  }
  if (S.editObject && S.painting) { applyVoxel(); return; }
  if (!S.drag) { if (S.editObject) updateVoxHover(); else hoverVox.visible = false; return; }
  const dx = e.clientX - S.drag.x, dy = e.clientY - S.drag.y;
  if (S.drag.mode === 'pan') { S.drag.x = e.clientX; S.drag.y = e.clientY; panCamera(dx, dy); }
  else if (S.drag.mode === 'orbit') { S.drag.x = e.clientX; S.drag.y = e.clientY; orbitView(dx, dy); }
...
canvas.addEventListener('pointerup', e => {
  ...
  if (measureActive()) {
    if (S.drag && !moved(e)) { if (S.drag.meas === 'freeze') freezeMeasure(); else if (S.drag.meas === 'clear') clearMeasure(); }
    S.drag = null; return;
  }
  if (S.editObject) {
    if (S.painting) { S.painting = false; updateChrome(); save(); }
  ...
  if (S.drag) {
    if (S.drag.mode === 'pan' && !moved(e)) {               // a click -> select / deselect
      ...
      if (id) { if (e.shiftKey) { S.selection.has(id) ? S.selection.delete(id) : S.selection.add(id); } else S.selection = new Set([id]); }
...
canvas.addEventListener('wheel', e => {
  e.preventDefault();
  goal.zoom *= (e.deltaY > 0 ? 1 / 0.95 : 0.95); goal.zoom = Math.max(6, Math.min(400, goal.zoom));
}, { passive: false });
