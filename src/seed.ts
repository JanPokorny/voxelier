...
export function seed(): SceneNode {
  ...
  for (let y = 1; y <= dh; y++) {
    for (let x = dx; x < dx + dw; x++) room.voxels.delete(key(x, y, 0));
  }
  ...
  for (const [lx, lz] of [[1, 1], [CS - 2, 1], [1, CS - 2], [CS - 2, CS - 2]]) {
    for (let y = 0; y < 9; y++) hv(lx, y, lz, 7); // legs
  }
  ...
  for (let y = 0; y < 9; y++) {
    for (let x = 0; x < 10; x++) {
      for (let z = 0; z < 10; z++) {
        if (y === 0 || x === 0 || x === 9 || z === 0 || z === 9) {
          pv(x, y, z, 9); // pot shell
        }
      }
    }
  }
  ...
  room.vis = "transparent"; // walls transparent by default
  desk.name = "Desk";
  desk.pos = { x: 0, y: 0, z: 0 };
  comp.name = "Computer";
  comp.pos = { x: 6, y: 18, z: 4 };
  const dws = newScene();
  dws.name = "Desk + Computer";
  dws.pos = { x: 6, y: 1, z: 30 };
  dws.children = [desk, comp];
  chair.name = "Chair";
  chair.pos = { x: 16, y: 1, z: 20 };
  chair.rot = 0; // in front of the desk, facing it
  plant.name = "Plant";
