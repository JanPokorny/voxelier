...
export const measureActive = () => S.editObject ? S.tool === 'measure' : S.measMode;
export const invalidateField = () => { S.measFieldCache = null; };
export function clearMeasure() { S.liveMeas = null; S.frozenMeas = []; renderMeasure(); }
...
export function measureField() {
  ...
  let toW;
  if (S.editObject) {
    for (const [k] of S.editObject.voxels) { const v = parseKey(k); acc(v.x, v.y, v.z); }
...
export function measureRef() { // voxel cell under the pointer, clamped into the field
  ...
  if (S.editObject) {
    ...
    const hits = S.pickMeshes.length ? raycaster.intersectObjects(S.pickMeshes, false) : [];
    if (hits.length) {
...
export function pointerMeasure() { const c = measureRef(); S.liveMeas = c ? measureAt(c) : null; renderMeasure(); }
export function freezeMeasure() { if (S.liveMeas && S.liveMeas.length) { S.frozenMeas.push(S.liveMeas); renderMeasure(); } }

export function renderMeasure() {
  const cont = document.getElementById('measure'); cont.innerHTML = ''; S.measLabels = [];
  const sets = []; if (S.liveMeas) sets.push({ s: S.liveMeas, fz: false }); for (const f of S.frozenMeas) sets.push({ s: f, fz: true });
  const pos = [], colr = [];
  for (const set of sets) for (const seg of set.s) {
    ...
    el.textContent = seg.len; cont.appendChild(el); S.measLabels.push({ el, w: seg.mid });
  }
