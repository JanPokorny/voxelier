// Re-packing a box list into (usually) fewer disjoint boxes, and the editor's
// entry point for it (reboxObject, run when the object editor closes — the only
// place box lists are edited; scene mode moves/rotates whole nodes in 90° steps
// and never touches their boxes). Editing degrades the decomposition:
// subtract() fragments boxes and only exact-face re-adds merge back (see absorb
// in boxes.ts) — this rebuilds it from scratch.
//
// The repack is boxes -> boxes with no per-voxel explosion. Coordinate
// compression makes that exact: collect the distinct face planes per axis;
// between adjacent planes occupancy and colour are constant, so the solid is a
// small grid of "super-cells" and any greedy growth over unit cells stops
// exactly on those planes anyway — greedy over super-cells yields the identical
// decomposition at O(planes³) cost, independent of world size (a 100000-wide
// slab is as cheap as a 10-wide one). Several axis orders are tried and the
// fewest-box result wins; realistic content packs in single-digit milliseconds,
// so it simply runs inline at editor close.
//
// This module is pure (no DOM / editor state), so selfcheck can exercise
// repackBoxes headless; the caller persists an applied result (see navigation).
...
// Re-pack an object's boxes in place; returns whether anything was applied (the
// caller then persists — a repack changes only the decomposition, so nothing
// needs remeshing and the save can fold into the current undo snapshot). Called
// when the object editor closes; any lifted selection has already been stamped
// back (clearSelection runs first), so the box list is a settled document state.
export function reboxObject(node: ObjectNode): boolean {
  if (node.boxes.length < MIN_BOXES || settled.has(node.boxes)) return false;
  ...
  settled.add(node.boxes); // already as good as the repack gets
  return false;
}
